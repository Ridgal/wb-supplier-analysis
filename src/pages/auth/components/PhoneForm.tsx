...
import { sendSmsCode } from '../../../redux/auth/actions';
...
const PhoneForm: React.FC = () => {
  ...
  const onSubmit = handleSubmit((data: FormValues) => {
    dispatch(sendSmsCode(data.login));
    if (status === 'success') {
      navigate('/register');
    }
    reset();
  ...
  return (
    <section className="min-h-full sm:min-h-0 flex flex-col justify-center sm:flex-none">
      <div className="flex justify-center sm:justify-start">
        <img
          className="mx-auto w-20 h-20 sm:w-10 sm:h-10 sm:mt-4 sm:ml-4 sm:mr-4"
          src={logo}
          alt="Your Company"
        />
      </div>
      <div className="min-h-full sm:mt-20">
        <div className="flex min-h-full flex-1 flex-col justify-center items-center px-6 pt-0 pb-12 sm:pt-0 lg:pt-0 lg:pb-4 lg:px-4">
          ...
          <div className="w-[40rem] mt-10 sm:mt-8 sm:mx-auto sm:w-full">
            ...
            <button className="flex items-center rounded-lg p-3 w-full border border-zinc-400 bg-backgroundLink">
              <img
                className="mr-4"
                src={telegramIcon}
                alt="telegramIcon"
